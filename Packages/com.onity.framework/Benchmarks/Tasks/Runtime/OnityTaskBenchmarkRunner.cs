...
namespace Onity.Benchmarks
{
    ...
    public sealed class OnityWhenAllBenchmarkRunner : MonoBehaviour
    {
        ...
        private void RunWorkerAllocationsCore(WhenAllReport report)
        {
            GC.GetAllocatedBytesForCurrentThread();
            long before = GC.GetAllocatedBytesForCurrentThread();
            ...
            for (int sample = 0; sample < k_samples; sample++)
            {
                OnityTaskCompletionSource[] first =
                    new OnityTaskCompletionSource[k_saturationOperations];
                OnityTaskCompletionSource[] second =
                    new OnityTaskCompletionSource[k_saturationOperations];
                OnityTask[] firstTasks = new OnityTask[k_saturationOperations];
                OnityTask[] secondTasks = new OnityTask[k_saturationOperations];
                OnityTask[] results = new OnityTask[k_saturationOperations];
                OnityTaskTracker.IsEnabled = false;
                for (int i = 0; i < k_saturationOperations; i++)
                {
                ...
                for (int i = 0; i < k_saturationOperations; i++)
                {
                    results[i] = OnityTask.WhenAll(firstTasks[i], secondTasks[i]);
                }
                ...
                for (int i = 0; i < k_saturationOperations; i++)
                {
                    ...
                    results[i].GetAwaiter().GetResult();
                }
            }
