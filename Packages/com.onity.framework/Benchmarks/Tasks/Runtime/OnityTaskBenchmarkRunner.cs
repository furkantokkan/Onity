...
namespace Onity.Benchmarks
{
    ...
    public sealed class OnityWhenAllBenchmarkRunner : MonoBehaviour
    {
        ...
        private bool m_captureCallstacks;
#if UNITY_EDITOR
        private bool m_profilerStateCaptured;
        private bool m_profilerWasEnabled;
        private bool m_profilerDriverWasEnabled;
        ...
        private bool m_lastAllocationValid;
        private long m_lastAllocationBytes;
        private WhenAllAllocationSample m_lastAllocationCallstackSample;
#endif
        ...
        private IEnumerator RunAllocations(WhenAllReport report)
        {
            ...
            for (int scenario = 0; scenario < report.scenarios.Length; scenario++)
            {
                ...
                for (int sample = 0; sample < k_samples; sample++)
                {
                    for (int turn = 0; turn < 2; turn++)
                    {
                        ...
                        ForceFullGc();
                        PrepareBatch();
                        m_captureCallstacks = m_activeLibrary == 0 &&
                            m_activeScenario.pending;
                        yield return CaptureAllocation(m_scheduleBatch);
                        m_captureCallstacks = false;
                        if (!m_lastAllocationValid)
                        {
                            throw new InvalidDataException(
                        ...
                        metric.sampleAllocatedBytes[sample] = m_lastAllocationBytes;
                        if (m_lastAllocationCallstackSample != null)
                        {
                            if (metric.allocationCallstackSamples == null)
                            {
                                metric.allocationCallstackSamples =
                                    new WhenAllAllocationSample[k_samples];
                            }

                            metric.allocationCallstackSamples[sample] =
                                m_lastAllocationCallstackSample;
                        }
                    }
                }
            ...
            report.allocationCounter =
                "Unity Profiler GC.Alloc metadata and pending Onity callstacks; "
                + "65,568/0-byte controls passed.";
        ...
        private IEnumerator CaptureAllocation(Action operation)
        {
            m_lastAllocationValid = false;
            m_lastAllocationBytes = 0;
            m_lastAllocationCallstackSample = null;
            string marker = k_scopePrefix
                + (m_markerSequence++).ToString(CultureInfo.InvariantCulture);
            Profiler.BeginSample(marker);
            try
            ...
            for (int wait = 0; wait < k_profilerReadFrames; wait++)
            {
                    ...
                    out long bytes, out int foundFrame))
                {
                    m_lastAllocationBytes = bytes;
                    m_lastCapturedFrame = foundFrame;
                    if (m_captureCallstacks)
                    {
                        WhenAllAllocationStack[] stacks =
                            ReadWhenAllAllocationCallstacks(foundFrame, marker,
                                out long stackBytes);
                        if (stackBytes != bytes)
                        {
                            throw new InvalidDataException(
                                "WhenAll callstack bytes did not match the marker bytes.");
                        }

                        m_lastAllocationCallstackSample = new WhenAllAllocationSample
                        {
                            markerBytes = bytes,
                            callstackBytes = stackBytes,
                            stacks = stacks
                        };
                    }
        ...
        private static WhenAllAllocationStack[] ReadWhenAllAllocationCallstacks(
            int frame, string marker, out long totalBytes)
        {
            totalBytes = 0;
            Dictionary<string, WhenAllAllocationStack> groups =
                new Dictionary<string, WhenAllAllocationStack>();
            List<ulong> addresses = new List<ulong>(32);
            for (int thread = 0; ; thread++)
            {
                using (RawFrameDataView data = ProfilerDriver.GetRawFrameDataView(frame, thread))
                {
                    ...
                    for (int sample = 0; sample < data.sampleCount; sample++)
                    {
                        ...
                        for (int child = sample + 1; child <= end; child++)
                        {
                            ...
                            string key = stack.Length > 0
                                ? stack.ToString() : "[no resolved callstack]";
                            long bytes = data.GetSampleMetadataAsLong(child, 0);
                            if (!groups.TryGetValue(key, out WhenAllAllocationStack group))
                            {
                                group = new WhenAllAllocationStack { stack = key };
                                groups.Add(key, group);
                            }
                        ...
                        List<WhenAllAllocationStack> sorted =
                            new List<WhenAllAllocationStack>(groups.Values);
                        sorted.Sort((left, right) => right.bytes.CompareTo(left.bytes));
                        return sorted.ToArray();
                    }
                }
            ...
            return new WhenAllAllocationStack[0];
        }
        ...
        private static void ClearAllocations(WhenAllReport report)
        {
            ...
            for (int scenario = 0; scenario < report.scenarios.Length; scenario++)
            {
                for (int library = 0; library < 2; library++)
                {
                    report.scenarios[scenario].results[library].bytesPerOperation = -1;
                    report.scenarios[scenario].results[library].sampleAllocatedBytes = null;
                    report.scenarios[scenario].results[library].allocationCallstackSamples = null;
                }
            }
        }
        ...
        private sealed class WhenAllMetric
        {
            ...
            public double[] sampleNanosecondsPerOperation;
            public long[] sampleAllocatedBytes;
            public WhenAllAllocationSample[] allocationCallstackSamples;
        }

        [Serializable]
        private sealed class WhenAllAllocationSample
        {
            public long markerBytes;
            public long callstackBytes;
            public WhenAllAllocationStack[] stacks;
        }

        [Serializable]
        private sealed class WhenAllAllocationStack
        {
            public string stack;
            public long bytes;
            public int allocations;
