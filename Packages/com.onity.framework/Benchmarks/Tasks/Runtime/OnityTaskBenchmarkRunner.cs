...
namespace Onity.Benchmarks
{
    ...
    /// <summary>
    /// Measures typed params-array WhenAll scheduling in a pinned Editor.
    /// Input arrays and tasks are prepared before each slice and consumed afterward.
    /// </summary>
    public sealed class OnityWhenAllBenchmarkRunner : MonoBehaviour
    {
        ...
        private const int k_harnessVersion = 3;
        ...
        private readonly OnityTaskCompletionSource<int>[][] m_onitySources =
            new OnityTaskCompletionSource<int>[k_operations][];
        private readonly OnityTask<int>[][] m_onityInputs =
            new OnityTask<int>[k_operations][];
        private readonly OnityTask<int[]>[] m_onityResults =
            new OnityTask<int[]>[k_operations];
        private readonly UniTaskCompletionSource<int>[][] m_uniTaskSources =
            new UniTaskCompletionSource<int>[k_operations][];
        private readonly UniTask<int>[][] m_uniTaskInputs =
            new UniTask<int>[k_operations][];
        private readonly UniTask<int[]>[] m_uniTaskResults =
            new UniTask<int[]>[k_operations];
        ...
        /// <summary>Queues the typed params-array WhenAll comparison in Play Mode.</summary>
        ...
        private static WhenAllReport CreateReport()
        {
            ...
            return new WhenAllReport
            {
                ...
                suite = "Typed params-array OnityTask WhenAll scheduling",
                generatedAtUtc = DateTime.UtcNow.ToString("O", CultureInfo.InvariantCulture),
                unityVersion = Application.unityVersion,
#if ENABLE_IL2CPP
                ...
                warmupBatches = k_warmupBatches,
                stopwatchFrequency = Stopwatch.Frequency,
                scope = "Same typed params-array WhenAll<int> call per operation. Two- and "
                    + "four-input arrays are created before the measured slice and reused. Each "
                    + "pending case has fresh unresolved completion-source inputs prepared outside "
                    + "the slice. The marker includes WhenAll scheduling and storing the returned "
                    + "value; input completion, continuation dispatch, ordered-result validation, "
                    + "and GetResult occur afterward. Completed controls use both libraries' "
                    + "FromResult values. Onity tracker-on matches its default; UniTask uses its "
                    + "default tracker setting. Samples include harness loop cost without "
                    + "subtraction. These Editor/Mono slices do not measure full lifecycle, "
                    + "frame time, or IL2CPP.",
                allocationCounter = "Unavailable: separate Profiler pass not run.",
                scenarios = new[]
                {
                    NewScenario("Pending two inputs; Onity tracker on", true, true, 2),
                    NewScenario("Completed two inputs; Onity tracker on", false, true, 2),
                    NewScenario("Completed four inputs; Onity tracker on", false, true, 4)
                }
            };
        }

        private static WhenAllScenario NewScenario(string name, bool pending,
            bool onityTrackerEnabled, int inputCount)
        {
        ...
        private void PrepareBatch()
        {
            ...
            if (m_activeLibrary == 0)
            {
                for (int i = 0; i < k_operations; i++)
                {
                    OnityTask<int>[] inputs = m_onityInputs[i];
                    if (inputs == null || inputs.Length != m_activeScenario.inputCount)
                    {
                        inputs = new OnityTask<int>[m_activeScenario.inputCount];
                        m_onityInputs[i] = inputs;
                    }

                    if (m_activeScenario.pending &&
                        (m_onitySources[i] == null ||
                         m_onitySources[i].Length != m_activeScenario.inputCount))
                    {
                        m_onitySources[i] =
                            new OnityTaskCompletionSource<int>[m_activeScenario.inputCount];
                    }
                    ...
                    for (int input = 0; input < inputs.Length; input++)
                    {
                        if (m_activeScenario.pending)
                        {
                            OnityTaskCompletionSource<int> source =
                                new OnityTaskCompletionSource<int>();
                            m_onitySources[i][input] = source;
                            inputs[input] = source.Task;
                        }
                        else
                        {
                            inputs[input] = OnityTask<int>.FromResult(input + 1);
                        }
                    }
                }
            }
            else
            {
                for (int i = 0; i < k_operations; i++)
                {
                    UniTask<int>[] inputs = m_uniTaskInputs[i];
                    if (inputs == null || inputs.Length != m_activeScenario.inputCount)
                    {
                        inputs = new UniTask<int>[m_activeScenario.inputCount];
                        m_uniTaskInputs[i] = inputs;
                    }

                    if (m_activeScenario.pending &&
                        (m_uniTaskSources[i] == null ||
                         m_uniTaskSources[i].Length != m_activeScenario.inputCount))
                    {
                        m_uniTaskSources[i] =
                            new UniTaskCompletionSource<int>[m_activeScenario.inputCount];
                    }
                    ...
                    for (int input = 0; input < inputs.Length; input++)
                    {
                        if (m_activeScenario.pending)
                        {
                            UniTaskCompletionSource<int> source =
                                new UniTaskCompletionSource<int>();
                            m_uniTaskSources[i][input] = source;
                            inputs[input] = source.Task;
                        }
                        else
                        {
                            inputs[input] = UniTask.FromResult(input + 1);
                        }
                    }
                }
            }
        ...
        private void ScheduleBatch()
        {
            if (m_activeLibrary == 0)
            {
                for (int i = 0; i < k_operations; i++)
                {
                    m_onityResults[i] = OnityTask.WhenAll<int>(m_onityInputs[i]);
                }
            }
            else
            {
                for (int i = 0; i < k_operations; i++)
                {
                    m_uniTaskResults[i] = UniTask.WhenAll<int>(m_uniTaskInputs[i]);
                }
            }
        }
            ...
            if (m_activeLibrary == 0)
            {
                for (int i = 0; i < k_operations; i++)
                {
                    ...
                    if (m_activeScenario.pending)
                    {
                        ...
                        {
                            throw new InvalidOperationException(
                                "Onity pending typed WhenAll completed before its inputs.");
                        }

                        for (int input = 0; input < m_activeScenario.inputCount; input++)
                        {
                            if (!m_onitySources[i][input].TrySetResult(input + 1))
                            {
                                throw new InvalidOperationException(
                                    "Onity pending typed WhenAll input completed more than once.");
                            }
                        }
                    }
                    ...
                    {
                        throw new InvalidOperationException(
                            "Onity completed-input typed WhenAll returned a pending task.");
                    }
                }
                ...
                for (int i = 0; i < k_operations; i++)
                {
                    ...
                    if (!awaiter.IsCompleted)
                    {
                        throw new InvalidOperationException("Onity typed WhenAll did not complete.");
                    }

                    ValidateResults(awaiter.GetResult(), m_activeScenario.inputCount);
                    for (int input = 0; input < m_activeScenario.inputCount; input++)
                    {
                        m_onityInputs[i][input] = default;
                        if (m_activeScenario.pending)
                        {
                            m_onitySources[i][input] = null;
                        }
                    }
            ...
            else
            {
                for (int i = 0; i < k_operations; i++)
                {
                    ...
                    if (m_activeScenario.pending)
                    {
                        ...
                        {
                            throw new InvalidOperationException(
                                "UniTask pending typed WhenAll completed before its inputs.");
                        }

                        for (int input = 0; input < m_activeScenario.inputCount; input++)
                        {
                            if (!m_uniTaskSources[i][input].TrySetResult(input + 1))
                            {
                                throw new InvalidOperationException(
                                    "UniTask pending typed WhenAll input completed more than once.");
                            }
                        }
                    }
                    ...
                    {
                        throw new InvalidOperationException(
                            "UniTask completed-input typed WhenAll returned a pending task.");
                    }
                }
                ...
                for (int i = 0; i < k_operations; i++)
                {
                    ...
                    if (!awaiter.IsCompleted)
                    {
                        throw new InvalidOperationException("UniTask typed WhenAll did not complete.");
                    }

                    ValidateResults(awaiter.GetResult(), m_activeScenario.inputCount);
                    for (int input = 0; input < m_activeScenario.inputCount; input++)
                    {
                        m_uniTaskInputs[i][input] = default;
                        if (m_activeScenario.pending)
                        {
                            m_uniTaskSources[i][input] = null;
                        }
                    }
        ...
        private static void ValidateResults(int[] results, int expectedCount)
        {
            if (results == null || results.Length != expectedCount)
            {
                throw new InvalidOperationException(
                    "Typed WhenAll returned an unexpected result count.");
            }
            ...
            for (int i = 0; i < expectedCount; i++)
            {
                ...
                {
                    throw new InvalidOperationException(
                        "Typed WhenAll returned results in the wrong order.");
                }
            }
        }
