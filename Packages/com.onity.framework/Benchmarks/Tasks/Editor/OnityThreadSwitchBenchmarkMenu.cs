...
{
    /// <summary>
    /// Editor entry points for the OnityTask thread-switch comparison benchmark.
    /// </summary>
    [InitializeOnLoad]
    public static class OnityThreadSwitchBenchmarkMenu
    {
        private const string k_resultsDirectory = "Packages/com.onity.framework/Benchmarks/Results";
        private const string k_latestJsonFileName = "onity-thread-switch-benchmark-latest.json";
        private const string k_pendingSessionKey = "Onity.Benchmarks.PendingThreadSwitchRun";
        private const string k_outputSessionKey = "Onity.Benchmarks.ThreadSwitchOutput";
        private const string k_commandLineSessionKey = "Onity.Benchmarks.ThreadSwitchCommandLine";
        private const string k_commandLineStartTicksSessionKey = "Onity.Benchmarks.ThreadSwitchCommandLineStartTicks";
        private const string k_outputArgument = "-onityThreadSwitchBenchmarkOutput";
        private const double k_commandLineTimeoutSeconds = 900d;

        static OnityThreadSwitchBenchmarkMenu()
        {
        ...
        [MenuItem("Onity/Benchmarks/Run OnityTask Thread Switch Benchmarks (Play Mode)")]
        private static void RunFromMenu()
        {
            if (!EditorApplication.isPlaying)
            {
                ...
                Debug.Log("Entering Play Mode. OnityTask thread-switch benchmark will run after Play Mode starts.");
                return;
            }
        ...
        [MenuItem("Onity/Benchmarks/Run OnityTask Thread Switch Benchmarks (Play Mode)", true)]
        ...
        /// <summary>
        /// Command-line entry point for running the thread-switch benchmark in Play Mode.
        ...
        public static void RunFromCommandLine()
        {
            ...
            {
                EditorApplication.isPlaying = true;
                Debug.Log("Entering Play Mode. OnityTask thread-switch command-line benchmark will run after Play Mode starts.");
                return;
            }
        ...
        private static void HandleCommandLineCompleted(string latestJson, Exception exception)
        {
            ...
            if (exception != null)
            {
                Debug.LogError($"OnityTask thread-switch command-line benchmark failed. Latest report path: {latestJson}");
            ...
            {
                Debug.LogError($"OnityTask thread-switch command-line benchmark did not write the expected report: {latestJson}");
                EditorApplication.Exit(1);
                return;
            }

            Debug.Log($"OnityTask thread-switch command-line benchmark completed. Latest report: {latestJson}");
        ...
        private static void HandleCommandLineTimeout()
        {
            ...
            Debug.LogError("OnityTask thread-switch command-line benchmark timed out.");
            ClearCommandLineSession();
            EditorApplication.Exit(1);
        }
